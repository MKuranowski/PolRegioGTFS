...
export interface TrainCalendar {
    ...
    dates: string[];
    train_ids: number[];
    date_train_map: { [index: string]: number };
}
...
export class Endpoint {
     ...
     */
    private async call(where: URL, accessors?: string[]): Promise<any> {
        // Timeout
        const now: number = Date.now();
        const delta = now - this.last_call;
        if (delta < this.pause) {
    ...
    async carriers(): Promise<Carrier[]> {
        return await this.call(new URL("carriers", this.base), ["carriers"]) as Carrier[];
    }
    ...
    async stations(): Promise<Station[]> {
        return await this.call(new URL("stations", this.base), ["stations"]) as Station[];
    }
     ...
     */
    async station_departures(station_slug: string): Promise<StationDetails<StationDepartureTarget>> {
        const url = new URL(station_slug, new URL("station_departures/", this.base));
        return await this.call(url, ["station_departure"]) as StationDetails<StationDepartureTarget>;
    }
     ...
     * @param slug slug of the station
     */
    async station_arrivals(station_slug: string): Promise<StationDetails<StationArrivalTarget>> {
        const url = new URL(station_slug, new URL("station_arrivals/", this.base));
        return await this.call(url, ["station_arrival"]) as StationDetails<StationArrivalTarget>;
    }
     ...
     */
    async trains(carrier_slug: string): Promise<CarrierTrainsList[]> {
        const url = new URL("carrier_trains_lists", this.base);
        url.searchParams.set("carrier", carrier_slug);
        return await this.call(url, ["carrier_trains_lists"]) as CarrierTrainsList[];
    }
     ...
     */
    async train_data(train_id: number): Promise<Train> {
        const url = new URL(train_id.toString(), new URL("trains/", this.base));
...
export async function get_stations_with_locations(url: string = "https://raw.githubusercontent.com/MKuranowski/PLRailMap/master/plrailmap.osm")
    ...
    // Cache for tag elements
    const tags: Map<string, string> = new Map();
    let stations: StationWithLocation[] = [];
